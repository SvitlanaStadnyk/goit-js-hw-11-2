...
var _fetchPhotos = require("./js/fetchPhotos");
...
searchForm.addEventListener("submit", async (e)=>{
    ...
    if (searchQueryResult === "") {
        ...
        return (0, _notiflix.Notify).failure("Sorry, there are no images matching your search query. Please try again.");
    }
    ...
    q = searchQueryResult;
    try {
        const results = await (0, _fetchPhotos.fetchPhotos)(searchQueryResult);
        ...
        if (hits.length === "") btnLoadMore.classList.remove("is-visible");
        (0, _notiflix.Notify).success(`'Hooray! We found ${results.totalHits} images.'`);
...
btnLoadMore.addEventListener("click", async ()=>{
    ...
    try {
        ...
        const { total , totalHits , hits  } = results;
        const totalPages = Math.ceil(totalHits / per_page);
        if (page >= totalPages) btnLoadMore.classList.remove("is-visible");
    } catch (error) {
        (0, _notiflix.Notify).failure("We're sorry, but you've reached the end of search results.");
    }
});
...
async function renderedPhotos(results) {
    const { hits  } = results;
    markupData.markup = hits.map((hit)=>`<a href="${hit.largeImageURL}"><div class="photo-card">
...
},{"./css/common.css":"97fCK","./js/fetchPhotos":"2acyu","notiflix":"5WWYd","simplelightbox":"9ydBq","simplelightbox/dist/simple-lightbox.min.css":"kaxSc","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"97fCK":[function() {},{}],"2acyu":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
...
async function fetchPhotos(searchQueryResult) {
    const { baseUrl , key , image_type , orientation , safesearch , order , page , per_page ,  } = pixabayAPI;
    pixabayAPI.page = `${pageN}`;
    const response = await (0, _axiosDefault.default).get(`${baseUrl}?key=${key}&q=${q}&image_type=${image_type}&orientation=${orientation}&safesearch=${safesearch}&order=${order}&page=${page}&per_page=${per_page}`);
    const results = response.data;
